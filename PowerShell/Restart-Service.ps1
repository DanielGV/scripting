...
Write-Log -Message "Trying to restart the service $serviceName" -Path $Path -Level "Info"

$service = Get-Service $serviceName

if (!$service) {
    Write-Log -Message "The service $serviceName was not found" -Path $Path -Level "Error"
    exit 1
}

if ($service.status -ne "Running") {
    Write-Log -Message "The service $serviceName was not running" -Path $Path -Level "Warn"
}

Restart-Service $service -Force

if ($service.status -ne "Running") {
    Write-Log -Message "The service $serviceName was NOT successfully restarted" -Path $Path -Level "Error"
    exit 1
}
else {
    Write-Log -Message "The service $serviceName was successfully restarted" -Path $Path -Level "Info"
}